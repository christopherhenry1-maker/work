    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (dayMeetings.length === 0) {
        ...
        dayMeetings.forEach((m, idx) => {
          ...
          // Mark conflicts
          if (m.hasConflict) {
            const conflictPara = cell.appendParagraph('⚠️ CONFLICT');
...
function getDocumentRepoLinks() {
  try {
    ...
    for (let i = 0; i < values.length; i++) {
      ...
      links.push({
        id: 'doc-repo-' + i,
        name: String(row[0] || ''),           // Column A - Description
        description: String(row[0] || ''),    // Column A - Description
        url: url,                              // Column D - Link to Materials
        link: url,                             // Column D - Link to Materials
        owner: String(row[2] || ''),          // Column C - Owner
        comment: String(row[6] || ''),        // Column G - Comment (was row[4], now row[6])
